...
namespace StormByte::Buffers {
	...
	class STORMBYTE_PUBLIC Consumer final: public Async {
		public:
			...
			/**
			 * @brief Waits until the buffer is marked with `EoF` or `Error` by its producer.
			 *
			 * This function blocks the calling thread until the buffer's status is updated to either `EoF` (End of File)
			 * or `Error`. It is useful for ensuring that all data has been processed or that an error condition has been
			 * encountered before proceeding.
			 *
			 * @note This function does not modify the buffer's contents.
			 */			
